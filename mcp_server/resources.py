...
def _parse_int(value: Any, default: int, min_value: int = 1) -> int:
    """Parse int with a safe fallback."""
    try:
        parsed = int(value)
        if parsed < min_value:
            return default
    ...
    async def read_resource(uri: str) -> str:
        ...
        try:
            if uri == "tencent://streamlive/channels":
                channels = await _call_in_thread(client.list_mdl_channels)
                return json.dumps({
            ...
            if uri == "tencent://streamlink/flows":
                flows = await _call_in_thread(client.list_streamlink_inputs)
                return json.dumps({
            ...
            if uri == "tencent://resources/all":
                resources = await _call_in_thread(client.list_all_resources)
            ...
            if uri == "tencent://resources/hierarchy":
            ...
            if uri == "tencent://schedules/upcoming":
                schedules = await _call_in_thread(schedule_manager.get_all_upcoming_schedules)
                return json.dumps({
            ...
            if uri == "tencent://streampackage/channels":
                channels = await _call_in_thread(client.list_streampackage_channels)
                return json.dumps({
            ...
            if uri == "tencent://css/domains":
                domains = await _call_in_thread(client.list_css_domains)
                return json.dumps({
                    "type": "css_domains",
                    "count": len(domains),
                    "domains": domains,
            ...
            if uri == "tencent://css/streams":
                streams = await _call_in_thread(client.list_css_streams)
                return json.dumps({
                    "type": "css_streams",
                    "count": len(streams),
                    "streams": streams,
            ...
            if uri.startswith("tencent://logs/integrated"):
                ...
                hours = _parse_int(params.get("hours", ["24"])[0], default=24)
                ...
                logs_data = await _call_in_thread(
                    client.get_integrated_logs,
            ...
            if uri == "tencent://alerts":
                ...
                for channel in running_channels:
                    ...
                    try:
                        alerts = await _call_in_thread(
                            get_channel_alerts, client, channel_id, channel_name
                        )
